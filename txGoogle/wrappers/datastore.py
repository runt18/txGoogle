...
from txGoogle.utils import getChunks
import time
...
class DatasetsWrapper(Datasets):
    ...
    def lookup(self, datasetId, prettyPrint=None, fields=None, quotaUser=None, oauth_token=None, key=None, userIp=None, alt=None, keys=None, transaction=None, readConsistency=None):
        if keys is not None:
            keys = [trySerialize(key) for key in keys]
    ...
    def query(self, datasetId, kinds=None, queryFilter=None, readConsistency=None, properties=None):
        ...
        if properties:
            ...
            projection = None
        return self.runQuery(datasetId, kinds=kinds, filters=queryFilter, readConsistency=readConsistency, projection=projection)
...
class BatchOperationTimeHandler(object):
    ...
    def _getChunks(self):
        return getChunks(self._OrmOperations.iteritems(), MAX_ITEMS_PER_COMMIT)
    ...
    def _onSuccess(self, result, cnt, startTs):
        ...
        msg = 'updates: {} index updates: {} execution time: {:.3f} sec'.format(cnt, indexUpdates, delta)
    ...
    def add(self, obj, operation):
        ...
        if not self._loopingCall.running:
            self._loopingCall.start(self.loopTime, now=False)
